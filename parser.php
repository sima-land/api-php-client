...
// Создаем объет логгер. Для логирование работы парсера.
...
// Парсер может одновременно обращатся к api в несколько потоков.
...
// Так же каждая сущность реализует интерфейс \Iterator. Соответственно можно получить все данные сущности следующим образом.
...
$parser = new \SimaLand\API\Parser\Parser([
    // Путь до файла с мета данными. Он необходим, для продолжение парсинга, если по какой-то причине парсер остановил свою работы.
